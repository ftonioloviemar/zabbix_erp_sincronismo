#!/usr/bin/env python3
"""Testes funcionais para validar a função parse_status_page."""
...
class TestParseStatusPageFuncional(unittest.TestCase):
    """Testes funcionais para validar todos os cenários da função parse_status_page."""
    
    def setUp(self):
        """Configuração inicial para cada teste."""
        self.maxDiff = None
    
    def criar_html_completo(self, dados_tabela):
        """Cria HTML completo com a estrutura real do sistema."""
        html_base = '''<div class='vista-role' style='position: absolute; top:0px; left:0px; bottom:4px; right:0px; overflow: auto;' role='vista' perfil='' cvista='2052' svistaaba='2902'>
   <div id='tbltblConCon2902' role='grid' class='div-grid-vistas' tempoupdate='00:00:00' cabecalho='Cód. Local||Filial||Ultimo Reg. Receb.||Data Ult. Reg. Receb.||Hora Ult. Reg. Receb.||Ultimo Reg. Env.||Qtde Falta Receber||Data Ult. Reg. Env.||Hora Ultimo Reg. Env.||Qtde Falta Enviar||Qtde Receber||Qtde Enviar||Log Filial p/ Sinc.||Data Atual||Log Sinc. p/ Filial||Hr Atual' style='position: absolute;top: 59px;left: 0px;right: 0px;bottom: 10px;' realce='null'>
     ...
     <div class="div-grid-table-vistas">
        ...
        return html_base.format(conteudo_dados=dados_tabela)
    ...
    def test_cenario_ok_simples(self):
        ...
        html = self.criar_html_completo(dados_html)
    ...
    def test_cenario_multiplas_filiais_misto(self):
        ...
        dados_html = f'''
                 ...
                 <tr>
                     <td coluna="Cód. Local"><div>3</div></td>
                     <td coluna="Filial"><div>FILIAL C</div></td>
                     <td coluna="Data Ult. Reg. Env."><div>{data_ok2}</div></td>
                     <td coluna="Hora Ultimo Reg. Env."><div>{hora_ok2}</div></td>
                     <td coluna="Log Filial p/ Sinc."><div>Erro na filial C</div></td>
                 </tr>'''
    ...
    def test_cenario_ambos_problemas(self):
        ...
        dados_html = f'''
                 <tr>
                     ...
                     <td coluna="Data Ult. Reg. Env."><div>{data_str}</div></td>
                     <td coluna="Hora Ultimo Reg. Env."><div>{hora_str}</div></td>
                     <td coluna="Log Filial p/ Sinc."><div>Erro de conexão</div></td>
                 </tr>'''
